...
The technique can be described very basically as: 
	\begin{itemize}
		\item A transducer produces an ultrasound signal focused on a region, $8mm\times2mm\times2mm$
...
In our 2D case we ignore the perfusion term and use a small adaption of the CN method using $T^i_{m,n}= T(i\delta t, m\delta x, n\delta y)$: $$F^i_{m,n} = \frac{1}{\rho c} \left( k\left[  \frac{(T^i_{m+1,n}-2T^i_{m,n}+T^i_{m-1,n})}{(\delta x)^2}+\frac{(T^i_{m,n+1}-2T^i_{m,n}+T^i_{m,n-1})}{(\delta y)^2}\right] +Q^i_{m,n}\right) $$
and \begin{equation}
